...
if not SUPABASE_URL or not SERVICE_KEY or not ANON_KEY:
    print(
        "[FATAL] Missing SUPABASE_URL, SUPABASE_ANON_KEY, or SUPABASE_SERVICE_ROLE_KEY in .env"
    )
    sys.exit(1)
...
def check_f2() -> None:
    ...
    if missing_rollbacks:
        ...
        record(
            "F2", True, f"All {len(up_files)} migrations have corresponding down.sql"
        )
...
def check_f4() -> None:
    ...
    if failures:
        ...
        record(
            "F4",
            True,
            f"{total_cols} columns. HALFVEC(768), GEOGRAPHY, TEXT[], TSVECTOR all correct",
        )
...
def check_f5() -> None:
    ...
    if r1.status_code in (200, 201) and r2.status_code == 409:
        record(
            "F5",
            True,
            f"First insert: {r1.status_code}, duplicate: {r2.status_code} (conflict)",
        )
    else:
...
def check_f7() -> None:
    ...
    record_skip(
        "F7",
        "pgmq queues not accessible via REST API. Verify via psql: SELECT pgmq.send('parse_queue', '{}')",
    )
...
def check_f8() -> None:
    ...
    record_skip(
        "F8",
        "cron.job not accessible via REST API. Verify via psql: SELECT * FROM cron.job",
    )
...
def check_f9() -> None:
    ...
    if col_count >= 14 and db_size > 0:
        record(
            "F9",
            True,
            f"{col_count} columns. db_size_bytes={db_size}. Columns: {', '.join(sorted(cols))}",
        )
    elif col_count < 14:
        record(
            "F9",
            False,
            f"Only {col_count} columns (need 14). Got: {', '.join(sorted(cols))}",
        )
    else:
...
def check_f10() -> None:
    ...
    if len(anon_rows) == 0:
        record(
            "F10",
            True,
            "Anon key returns 0 rows for status='raw' (RLS blocks correctly)",
        )
    else:
        record(
            "F10",
            False,
            f"Anon key returned {len(anon_rows)} raw rows (RLS NOT blocking)",
        )
...
def check_f11() -> None:
    ...
    if len(anon_rows) >= 1:
        record(
            "F11",
            True,
            f"Anon key returns {len(anon_rows)} row(s) for status='ready' (RLS allows)",
        )
    else:
        record(
            "F11",
            False,
            "Anon key returned 0 rows for status='ready' (RLS too restrictive)",
        )
...
def check_f12() -> None:
    ...
    if found == expected and all_active and len(sources) == 4:
        record(
            "F12", True, f"4 sources: {', '.join(sorted(names))}. All is_active=true"
        )
    else:
        record(
            "F12",
            False,
            f"Found {len(sources)} sources: {names}. All active: {all_active}. Missing: {expected - found}",
        )
...
def check_f13() -> None:
    ...
    record_skip(
        "F13",
        "pg_class.reloptions not accessible via REST. Verify via psql: SELECT reloptions FROM pg_class WHERE relname='jobs'",
    )
